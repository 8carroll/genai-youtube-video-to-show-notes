...
def append_summary_to_file(summary, title, url, category):
        ...
        if video_url:
            ...
            try:
                ...
                if transcript_text:
                    ...
                    summary, category = summarize_transcript(transcript_text)
                    if summary:
                        ...
                        append_summary_to_file(summary, yt.title, video_url, category)
            except Exception as e:
    ...
    if st.button("Summarize Content"):
        if url:
            try:
                ...
                summary, category = summarize_transcript(text_content)
                if summary:
                    st.write("Summary:")
                    st.write(summary)
                    copy_text = f"* [{page_title}]({url}) - {summary}\n\nCategory: {category}"
                    st.text_area("Copy the Markdown text below:", copy_text, height=150)
                    append_summary_to_file(summary, page_title, url, category)
            except Exception as e:
                st.error(f"An error occurred: {e}")
        else:
